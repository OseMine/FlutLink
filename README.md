...
- **Phase 3 (in progress):** Chunked uploads/downloads with progress events (`app.emit`), drag & drop upload, select-all and bulk download/delete in the file browser. `resources`/`parts` dual-pane workflows (pairing + split view) are done. Remaining: symlink/virtual-link resolution.
