...
frontend). A companion **Android client** (`android/`) ports the same
features to Kotlin + Jetpack Compose.
...
>
> **The Android client is a port of the desktop app to Kotlin, generated by
> opencode** — see [`android/README.md`](android/README.md). It mirrors the
...
├── android/                    # FlutLink mobile client (Kotlin + Jetpack Compose)
│   │                           # Port of the desktop app to Kotlin, generated by opencode
│   ├── app/                    # Android app module (com.flutcloud.flutlink)
│   │   └── src/main/java/com/flutcloud/flutlink/
│   │       ├── data/           # FlutCloudApi (OCS), WebDavApi (PROPFIND/XML), models
│   │       ├── core/           # AccountStore (EncryptedSharedPreferences), SettingsStore
│   │       ├── ui/             # Compose screens: Login, Files, Admin, Settings + M3 theme
│   │       └── AppContainer.kt # Service locator (OkHttp client, stores, session)
│   └── gradle/libs.versions.toml # Version catalog (AGP 8.13, Compose BOM 2025.06)
...
- The Android client (`android/`) bakes the same URL into
