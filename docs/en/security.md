...
- Admin commands (`admin_list_users`, `admin_get_user`,
  `admin_set_user_quota`, `admin_edit_user`) are only allowed for admin
  accounts.
...
- Frontend origin: only the packaged frontend / dev server are loaded; `csp`
  is currently `null` — keep it in mind before adding remote content.
