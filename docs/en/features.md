...
A companion **mobile client** (`kmp/`) ports the desktop feature set to
Kotlin Multiplatform (Material 3) — Android, JVM and iOS targets in one
codebase. It is generated by opencode, mirrors the desktop app and is not a
separate product. It enforces the same FlutCloud-only policy: the `flutcloud`
capability is probed before any account is accepted, and when a server URL is
compiled into the build (`FLUTCLOUD_URL` / `-PflutcloudUrl`) the login screen
locks the URL field.
...
- **iOS** — targets + Xcode shell (`kmp/iosApp/`) are set up and built in CI
  (unsigned IPA); full UI parity with the Android target is follow-up work.

Not ported (desktop-only for now): two-way sync (deliberate product decision,
see `kmp/README.md`), admin impersonation, and tray/CLI behaviour.

See [`kmp/README.md`](../../kmp/README.md) for the build, signing and CI
details.
