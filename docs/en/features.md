...
- **Files** — WebDAV browsing (PROPFIND), upload via the Storage Access
  Framework, download, folder creation, rename, delete, global search (SEARCH),
  `resources`/`parts` virtual-link pairing and public share-link creation.
- **Admin** — OCS user administration: list/search, create, delete,
  enable/disable and quota presets.
...
Not ported (desktop-only for now): two-way sync, group management, admin
impersonation, and tray/CLI behaviour.
