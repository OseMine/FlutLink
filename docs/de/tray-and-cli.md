...
- `--url` emittiert `flutlink:cli-open`; `App.vue` hört zu und öffnet
  ...
  den `get_flutcloud_url`-Command, der `FLUTCLOUD_URL` aus der `.env` bzw. dem
  Build liest, siehe `src-tauri/src/flutcloud.rs`) — FlutLink ist ein
  dedizierter Client und verbindet sich nie woanders hin.
